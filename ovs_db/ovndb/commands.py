...
from ovs_db.ovsdb.native import commands
...
class DelLSwitchCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Logical Switch %s does not exist") % self.name
...
class AddLSwitchPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            msg =  i18n.translate("Logical Switch %s does not exist") % self.lswitch
            raise RuntimeError(msg)
        if self.may_exist:
...
class SetLSwitchPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Logical Switch Port %s does not exist") % self.lport
            raise RuntimeError(msg)
...
class DelLSwitchPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Port %s does not exist") % self.lport
            raise RuntimeError(msg)
...
class UpdateLRouterCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Logical Router %s does not exist") % self.name
            raise RuntimeError(msg)
...
class AddLRouterPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        ...
        try:
            ...
            msg =  i18n.translate("Logical Router %s does not exist") % self.lrouter
            raise RuntimeError(msg)
        try:
            ...
            # correctly.
            msg =  i18n.translate("Logical Router Port with name \"%s\" "
...
class UpdateLRouterPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Logical Router Port %s does not exist") % self.name
            raise RuntimeError(msg)
...
class SetLRouterPortInLSwitchPortCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            msg =  i18n.translate("Logical Switch Port %s does not "
...
class DelAddrSetCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg =  i18n.translate("Address set %s does not exist. "
...
class DelDHCPOptionsCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        if self.row_uuid not in self.api._tables['DHCP_Options'].rows:
            if self.if_exists:
                return
            msg =  i18n.translate("DHCP Options row %s does not exist") % self.row_uuid
            raise RuntimeError(msg)
...
class AddNATRuleInLRouterCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        ...
        for col, val in self.columns.items():
            setattr(row, col, val)
        # TODO(chandrav): convert this to ovs_db transaction mutate
        lrouter.verify('nat')
...
class DeleteNatIpFromLRPortPeerOptionsCommand(commands.BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            msg =  i18n.translate("Logical Switch Port %s does not exist") % self.port
            raise RuntimeError(msg)
