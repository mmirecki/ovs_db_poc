...
from ovs_db.compatibility.logging import logging
from ovs_db.compatibility.i18n import i18n
from ovs_db.compatibility import utils
from ovs_db.compatibility.config import config
from ovs_db.compatibility import constants as ovn_const

# from neutron_lib import exceptions as n_exc
# from oslo_log import log
import tenacity

from ovs_db.ovsdb import impl_idl
from ovs_db.ovsdb.native import idlutils
...
from ovs_db.ovndb import commands as cmd
from ovs_db.ovndb import ovn_api
from ovs_db.ovndb import ovsdb_monitor
from ovs_db.ovndb import vlog
...
class OvsdbConnectionUnavailable(utils.ServiceUnavailable):
    message =  i18n.translate("OVS database connection to %(db_schema)s failed with error: "
...
def get_ovn_idls(driver, trigger):
    ...
    def get_ovn_idl_retry(cls, driver, trigger):
        LOG.info( i18n.translate('Getting %(cls)s for %(trigger)s with retry'),
...
def get_connection(db_class, trigger=None):
    ...
    if trigger and type(trigger.im_class).__name__ == 'OvnWorker':
        cls = ovsdb_monitor.OvnConnection
    else:
        cls = ovsdb_monitor.OvnBaseConnection

    if db_class == OvsdbNbOvnIdl:
        return cls(config.get_ovn_nb_connection(),
                   config.get_ovn_ovsdb_timeout(), 'OVN_Northbound')
    elif db_class == OvsdbSbOvnIdl:
        return cls(config.get_ovn_sb_connection(),
                   config.get_ovn_ovsdb_timeout(), 'OVN_Southbound')
...
class OvsdbNbOvnIdl(ovn_api.API):
    ...
    def __init__(self, driver, trigger=None):
        ...
        try:
            ...
            self.ovsdb_timeout = config.get_ovn_ovsdb_timeout()
    ...
    def delete_lswitch(self, lswitch_name=None, ext_id=None, if_exists=True):
        if lswitch_name is not None:
            ...
            raise RuntimeError( i18n.translate("Currently only supports delete "
                                 "by lswitch-name"))
    ...
    def delete_lswitch_port(self, lport_name=None, lswitch_name=None,
        ...
        if lport_name is not None:
            ...
            raise RuntimeError( i18n.translate("Currently only supports "
    ...
    def get_lrouter_nat_rules(self, lrouter_name):
        try:
            ...
            msg =  i18n.translate("Logical Router %s does not exist") % lrouter_name
    ...
    def check_for_row_by_value_and_retry(self, table, column, match):
        try:
            ...
            msg = ( i18n.translate("%(match)s does not exist in %(column)s of %(table)s")
...
class OvsdbSbOvnIdl(ovn_api.SbAPI):
    ...
    def _get_chassis_physnets(self, chassis):
        ...
        mapping_dict = utils.parse_mappings(bridge_mappings.split(','))
    ...
    def get_chassis_data_for_ml2_bind_port(self, hostname):
        try:
            ...
            msg =  i18n.translate('Chassis with hostname %s does not exist') % hostname
            raise RuntimeError(msg)
