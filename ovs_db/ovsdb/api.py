...
# Move to constants
interface_map = {
    'ovs_db': 'neutron.agent.ovsdb.impl_ovs.Ovsdbovs',
...
class API(object):
    ...
    def get(context, iface_name=None):
        ...
        iface = utils.import_class(
            interface_map[iface_name or config.get_ovsdb_type()])
    ...
    def add_manager(self, connection_uri):
        ...
        :type connection_uri: string, see ovs_db-ovs_db manpage for format
    ...
    def db_find(self, table, *conditions, **kwargs):
        ...
        :type conditions: 3-tuples containing (column, operation, match)
                          ...
                          See the ovs_db-ovs_db man page for more operations
        :param columns:   Limit results to only columns, None means all columns
        :type columns:    list of column names or None
        :returns:         :class:`Command` with [{'column', value}, ...] result
        """
    ...
    def set_controller(self, bridge, controllers):
        ...
        :type controllers:  list of strings, see ovs_db-ovs_db manpage for format
        :returns:           :class:`Command` with no result
        """
...
def py_to_val(pyval):
    """Convert python value to ovs_db-ovs_db value argument"""
