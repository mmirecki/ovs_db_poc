...
from ovs_db.ovsdb import api
from ovs_db.ovsdb.native import commands as cmd
from ovs_db.ovsdb.native import connection
from ovs_db.ovsdb.native import idlutils
from ovs_db.ovsdb.native import vlog
...
class VswitchdInterfaceAddException(utils.NeutronException):
    message = i18n.translate("Failed to add interfaces: %(ifaces)s")
    ...
    def commit(self):
        ...
        try:
            ...
            raise api.TimeoutException(
                i18n.translate("Commands %(commands)s exceeded timeout %(timeout)d "
        ...
        while True:
            ...
            for i, command in enumerate(self.commands):
                ...
                try:
                    ...
                    with utils.save_and_reraise_exception() as ctx:
                        txn.abort()
            ...
            if status == txn.TRY_AGAIN:
                ...
                msg = i18n.translate("OVSDB Error: %s") % txn.get_error()
                if self.log_errors:
                    LOG.error(msg)
                if self.check_error:
...
class NeutronOVSDBTransaction(Transaction):
    ...
    def post_commit(self, txn):
        ...
        # ovs_db-vsctl only logs these failures and does not return nonzero
        try:
            self.do_post_commit(txn)
        except Exception:
            LOG.exception(i18n.translate("Post-commit checks failed"))
...
class OvsdbIdl(api.API):
    ...
    ovsdb_connection = connection.Connection(config.get_ovsdb_connection(),
                                             config.get_ovs_vsctl_timeout(),
                                             'Open_vSwitch')
