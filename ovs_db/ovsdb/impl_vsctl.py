...
from ovs_db.compatibility.executils import executils
...
class Transaction(ovsdb.Transaction):
    ...
    def run_vsctl(self, args):
        full_args = ["ovs_db-vsctl"] + self.opts + args
        try:
            # We log our own errors, so never have utils.execute do it
            return executils.execute(full_args, run_as_root=True,
                                     log_fail_as_error=False).rstrip()
        except Exception as e:
            with utils.save_and_reraise_exception() as ctxt:
                if self.log_errors:
                    LOG.error(i18n.translate("Unable to execute %(cmd)s. "
...
class MultiLineCommand(BaseCommand):
    """Command for ovs_db-vsctl commands that return multiple lines"""
    @property
    def result(self):
        return self._result
...
class DbCommand(BaseCommand):
    ...
    def result(self, raw_result):
        ...
        try:
            json = utils.loads(raw_result)
        except (ValueError, TypeError) as e:
            # This shouldn't happen, but if it does and we check_errors
            # log and raise.
            with utils.save_and_reraise_exception():
                LOG.error(i18n.translate("Could not parse: %(raw_result)s. "
                              "Exception: %(exception)s"),
...
class DbCreateCommand(BaseCommand):
    def __init__(self, context, opts=None, args=None):
        ...
        self.record_id = "@%s" % utils.generate_uuid()
