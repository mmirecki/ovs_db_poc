    ...
    def execute(self, check_error=False, log_errors=True):
        try:
            ...
            with utils.save_and_reraise_exception() as ctx:
                if log_errors:
                    LOG.exception(i18n("Error executing command"))
                if not check_error:
                    ctx.reraise = False
...
class RemoveManagerCommand(BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            msg =i18n.translate("Manager with target %s does not exist") % self.target
            LOG.error(msg)
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                ...
                msg = i18n.translate("Bridge %s does not exist") % self.name
                LOG.error(msg)
                raise RuntimeError(msg)
        self.api._ovs.verify('bridges')
...
class DelPortCommand(BaseCommand):
    ...
    def run_idl(self, txn):
        try:
            ...
            if self.if_exists:
                return
            msg = i18n.translate("Port %s does not exist") % self.port
            raise RuntimeError(msg)
        if self.bridge:
            br = idlutils.row_by_value(self.api.idl, 'Bridge', 'name',
                                       self.bridge)
        ...
        if port.uuid not in br.ports and not self.if_exists:
            ...
            msg = i18n.translate("Port %(port)s does not exist on %(bridge)s!") % {
