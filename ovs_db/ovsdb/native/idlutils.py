...
from ovs_db.ovsdb.native import helpers
...
class RowNotFound(utils.NeutronException):

    message =  i18n.translate("Cannot find %(table)s with %(col)s=%(match)s")
...
def row_by_record(idl_, table, record):
    ...
    if rl.table is None:
        raise ValueError( i18n.translate("Table %s can only be queried by UUID") % table)
    ...
    if rl.uuid_column:
        ...
        if len(rows) != 1:
            raise RowNotFound(table=table, col=i18n.translate('record'), match=record)
        row = rows[0]
    return row
...
def _get_schema_helper(connection, schema_name):
    ...
    if err:
        raise Exception( i18n.translate("Could not connect to %s") % connection)
    rpc = jsonrpc.Connection(strm)
    req = jsonrpc.Message.create_request('get_schema', [schema_name])
    err, resp = rpc.transact_block(req)
    rpc.close()
    if err:
        raise Exception( i18n.translate("Could not retrieve schema from %(conn)s: "
...
def get_schema_helper(connection, schema_name, retry=True):
    try:
        ...
        with utils.save_and_reraise_exception(reraise=False) as ctx:
            if not retry:
                ctx.reraise = True
...
def wait_for_change(_idl, timeout, seqno=None):
    ...
    while _idl.change_seqno == seqno and not _idl.run():
        ...
        ovs_poller.block()
        if time.time() > stop:
            raise Exception( i18n.translate("Timeout"))
...
def get_column_value(row, col):
    ...
    # Idl returns lists of Rows where ovs_db-vsctl returns lists of UUIDs
    if isinstance(val, list) and len(val):
        if isinstance(val[0], idl.Row):
            val = [v.uuid for v in val]
        col_type = row._table.columns[col].type
        # ovs_db-vsctl treats lists of 1 as single results
...
def condition_match(row, condition):
    ...
    if type(match) is not type(val):
        ...
        if (not all([match, val]) and
            ...
            raise ValueError(
                 i18n.translate("Column type and condition operand do not match"))
